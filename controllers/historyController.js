...
const HistoryController = {
  getHistories: async (req, res) => {
    try {
      ...
      const result = await historyService.getHistories(req.params.accountId, page, pageSize, sortField, sortOrder);
      res.json(result);
    } catch (err) {
  ...
  updateHistory: async (req, res) => {
    try {
      const updatedHistory = await historyService.updateHistory(req.params.id, req.body);
